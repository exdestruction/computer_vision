...
int main()
{
	...
	for(const auto& image: images)
	{
		...
		cv::GaussianBlur(image_matrix, image_matrix, cv::Size(3, 3), 0, 0);
		...
		{
			cv::adaptiveThreshold(image_matrix, image_adaptive_threshold, 255, cv::ADAPTIVE_THRESH_GAUSSIAN_C, cv::THRESH_BINARY, 555, 5);
		...
		else if (image->get_name() == "redbull.bmp")
		{
			...
			cv::adaptiveThreshold(illuminate(image_matrix, 0.5), image_adaptive_threshold, 255, cv::ADAPTIVE_THRESH_GAUSSIAN_C, cv::THRESH_BINARY, 555, 15);
			image->add_derived_image(name + " adaptive threshold", image_adaptive_threshold);
		}

		//canny filter applyied
		int lower_threshold = 35;
		int upper_threshold = 3 * lower_threshold;
		cv::GaussianBlur(image_matrix, image_canny, cv::Size(3, 3), 0, 0);
		...
		cv::Canny(image_canny, image_canny, lower_threshold, upper_threshold, 3, true);
